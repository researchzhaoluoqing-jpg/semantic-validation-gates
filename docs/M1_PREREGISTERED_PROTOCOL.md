...
| 2026-07-19 | D5: E2 KS-vs-Beta audit statistic declared miscalibrated (post-hoc, null-check evidence). Randomized tie-breaking does **not** restore the law on the v8 pool (KS p≈0); the same KS rejects ideal continuous i.i.d. scores under the shared-pool procedure and even under fresh draws (test-binomial noise, p=0.0002), while analytically exact conditional coverage passes (p=0.64). | The Beta refinement describes exact conditional coverage, which finite audits observe only through binomial convolution; a valid audit must test against the convolved law. | Exceedance-bound conclusions unaffected (held in all three runs); the KS line in E2 reports is reinterpreted as an audit-statistic property, not evidence about Thm 4.2 |
